...
# Función para cargar datos
def cargar_datos(usuario, tree_proyectos, tree_tareas):
    proyectos, tareas = obtener_proyectos_y_tareas(usuario.id_usuario)
...
def mostrar_panel_principal(usuario):
    ...
    cargar_datos(usuario, tree_proyectos, tree_tareas)
    root.mainloop()
...
def crear_proyecto_window(usuario):
    def guardar_proyecto():
        ...
        try:
            ...
            messagebox.showerror("Error", "Las fechas no tienen un formato válido. Usa el formato (YYYY-MM-DD)")
    ...
    label_fecha_fin = tk.Label(create_window, text="Fecha de Fin (opcional):")
    label_fecha_fin.pack(pady=5)
    entry_fecha_fin = tk.Entry(create_window)
    entry_fecha_fin.pack(pady=5)
...
def crear_tarea_window(usuario):
    def guardar_tarea():
        ...
        try:
            ...
            messagebox.showerror("Error", "La fecha de vencimiento no tiene un formato válido. Usa el formato (YYYY-MM-DD)")
